                            ...
                            <tr>
                                <?php foreach ($fields as $field_name => $field_display): ?>
                                    <th <?php if ($order_column == $field_name) echo "class=sort_$order_type " ?>>
                                        <?php echo anchor('staff/view/'.$search.'/'.$limit.'/'.$field_name.'/'.
                        ...
                        <tbody>
							<?php foreach ($view_data as $key => $data): ?>
							  <tr>
								...
								<td><?php echo $data['SL_UID']; ?></td>
								<td><?php echo $data['SL_IN']; ?></td>
								<td><?php echo $data['SL_OUT']; ?></td>
                               <!--  <?php foreach ($fields as $field_name => $field_display): ?>
                                    <td>
								...
								<td>
                                    <button type="button" data-qr-title="<?php echo $data['U_ID'].' - '.$data['U_LName'].', '.$data['U_FName']; ?>" class="btn btn-primary btn-sm qr-code-context" title="view QR code" data-qr-url="http://api.qrserver.com/v1/create-qr-code/?data=<?php echo 'Staff-'.$data['S_ID']; ?>&size=100x100">
