...
$this->db->or_like('SL_IN', $search); 
$this->db->or_like('SL_OUT', $search); 
}

$this->db->join($this->table2, 'staff_log.S_ID = users.U_ID', 'left');
$order_type = ($order_type == 'desc') ? 'desc' : 'asc';
$order_by = array('U_LName','SL_UID','SL_IN','SL_OUT', 'SL_ID');
$order_column = (in_array($order_column, $order_by)) ? $order_column : 'SL_ID';
...
function count_all($search){
	if($search){ 
	$this->db->like('S_ID', $search); 
$this->db->or_like('SL_UID', $search); 
$this->db->or_like('SL_IN', $search); 
$this->db->or_like('SL_OUT', $search); 
	}
