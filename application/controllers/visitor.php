...
class Visitor extends CI_Controller{
 ...
 public function view($search = 0, $limit = 10, $order_column = 'VL_ID', $order_type = 'desc', $offset = 0)
 {
...
if (isset($_GET['search'])) {
	...
	$data['total_rows']= $this->visitor_model->count_all($search);
	$config['base_url'] = site_url($data['class'].'/'.$data['method'].'/'.$search.'/'.$limit.'/'.$order_column.'/'.$order_type.'/');
	$data['view_data']= $this->visitor_model->get_paged_list($search,$limit, $offset, $order_column, $order_type);
  ...
  $this->load->view("visitor.php", $data);
